    ...
    def start_besgo(self, current_time, set_relay8, set_plc_out, setting_mode, setting_selection,plc_all_in):
        if int(setting_selection[0]['backwash']) == 1:
            ...
            if str(besgo_setting_json[0]['backwash_mode']) == "1":
                with open('/home/pi/hottub_cocoon/txt_file/status_besgo.txt','w')  as write_status_besgo:
                    write_status_besgo.write("True")
                    write_status_besgo.close()
                #ปิดการทำงาน อ่านค่า Sensor 
                with open('/home/pi/hottub_cocoon/txt_file/stop_loading_in_tank.txt','w')  as write_loading_in_tank:
                    write_loading_in_tank.write("True")
                    write_loading_in_tank.close()
                #อ่านค่า counter เพื่อดับเครื่องทั้งหมดเพื่อให้เติมน้ำเข้าแท้ง
                read_counter_before_backwash = open("/home/pi/hottub_cocoon/txt_file/counter_start_before_backwash.txt","r")
                ...
                if status_backwash == "False":
                    for i in range(len(besgo_json)):
                        for j in range(len(besgo_json[i][0])):
                            if besgo_json[i][0][j] == day.upper():
                                ...
                                #                 write_status_besgo = open('/home/pi/hottub_cocoon/txt_file/status_besgo.txt','w')
                                #                 write_status_besgo.write("True")
                    ...
                    if int(number_counter_before_backwash) <= (int(besgo_setting_json[0]['backwash_countdown']) * 60):
                        ...
                        with open('/home/pi/hottub_cocoon/txt_file/stop_loading_in_tank.txt','w')  as write_loading_in_tank:
                            write_loading_in_tank.write("False")
                            write_loading_in_tank.close()
                        with open('/home/pi/hottub_cocoon/txt_file/status_besgo_start_counter.txt','w') as status_besgo_start_counter:
                        ...
                        #ตั้งสถานะให้อ่าน counter backwash กำลังทำงานจาก Thread 
                            ...
                            with open('/home/pi/hottub_cocoon/txt_file/status_besgo.txt','w') as write_status_besgo:
                                write_status_besgo.write("False")
                                write_status_besgo.close()
                            with open('/home/pi/hottub_cocoon/txt_file/counter_start_before_backwash.txt','w') as write_counter_in_tank:
                            ...
                            with open('/home/pi/hottub_cocoon/txt_file/counter_backwash_working.txt','w') as write_counter:
                                ...
                                write_counter.close()
                            with open('/home/pi/hottub_cocoon/txt_file/status_besgo_start_counter.txt','w')  as status_open_backwash:
